...
QVariant PathInfo::operator[] ( int column ) const
{
    switch ( column )
    {
        case 0:
 ...
 *
 ***************************************/
void PathInfo::openPath( bool ignoreIsOpenable ) const
{
...
void PathInfo::checkPathValidity()
{
    ...
    // check if exists now
    m_isValid = m_file_info.exists( m_path );
    qDebug() << "\ncurrent path: " << m_path << "file_info.exist yelds: " << m_isValid;
